...
export function Form() {
  const { user } = useAuth();
  const usermodifying = QueriesSimple_({
    coleccion: "users",
    parametro: "uid",
    busqueda: user.uid,
  }).props.children[0];
  ...
  const handleSubmit = (aux) => {
    swal({
      title:
        "Despues actualizar la información de este usuario, se debe recargar la aplicación para poder observar los cambios. ¿Desea realizar esta acción?",
      icon: "warning",
      buttons: ["No", "Si"],
    }).then((respuesta) => {
      if (respuesta) {
    ...
    <>
      ...
      >
        ...
        <div className={st.panel}>
          ...
          <br />
          {usermodifying !== undefined &&
            usermodifying.rol === "administrador" && (
              <div className={st.er}>
                <Lists
                  value_={userData?.info?.rol}
                  leyend="Rol"
                  name_="rol"
                  listar={["administrador", "usuario"]}
                  handleChange={hf}
                />
              </div>
            )}
