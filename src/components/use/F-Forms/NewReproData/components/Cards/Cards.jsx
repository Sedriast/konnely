...
export function Cards({ id, litterPrueba, stages }) {
  ...
  const autentication = (e, valor) => {
    if (valor[0] !== undefined) {
      if (
        valor[0].idPadre === basicData.info.idPadre ||
        valor[0].idMadre === basicData.info.idMadre
      ) {
        swal({
        ...
        swal({
          ...
          if (respuesta) {
            AddReproductiveCycle(
              formatCycleReproductive(e, litterPrueba, user)
            );
          }
        });
      }
      ...
      swal({
        title:
          "No existe un registro de este conejo, ¿segura o seguro que desea guardar este ciclo reproductivo?",
        dangerMode: true,
        icon: "warning",
        buttons: ["Cancelar", "Aceptar"],
      }).then((respuesta) => {
          ...
          const valor = males.filter(function (element) {
            if (
              element.id
                .toString()
                .toLowerCase()
                .includes(e.target.Macho.value.toLowerCase())
            ) {
              return element;
            } else {
              return null;
